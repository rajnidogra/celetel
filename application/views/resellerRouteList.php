									...
									<div class="row">
										...
										<div class="col-xl-12">
                                            ...
                                            <div class="card-header flex-wrap">
                                                <div>
                                                    <h4 class="card-title">Reseller Route List</h4>
                                                </div>
                                            </div>
                                            <div class="card-body">
                                            <form action="<?php echo base_url('telco/saveResellerRoute');?>" method="POST">
                                                <div class="basic-form">
                                                        <div class="row">
                                                            <div class="mb-3 col-md-3">
                                                                <label class="form-label" for="user_id">Select Reseller</label>
                                                                <select class="form-control" id="user_id" name="user_id" required="required">
                                                                    <option value="">Select User</option>
                                                                    <?php foreach ($userList as $type ) { ?>
                                                                        <option value="<?php echo $type['id']; ?>"><?php echo $type["name"]."(".$type["email"].")"; ?></option>
                                                                    <?php } ?>
                                                                </select>
                                                            </div>
                                                            <div class="mb-3 col-md-3">
                                                                <label class="form-label" for="routing_type">Select Group</label>
                                                                <select class="form-control" id="route_id" name="route_id" required="required">
                                                                    <option value="">Select Group</option>
                                                                    <?php foreach ($routeList as $type ) { ?>
                                                                        <option value="<?php echo $type['id']; ?>"><?php echo $type["group_id"]; ?></option>
                                                                    <?php } ?>
                                                                </select>   
                                                            </div>
                                                        </div>
    ...
    ***********************************-->
	...
	<script>
        ...
        function getdata(){
            ...
            $('#example').DataTable({
                "ajax": {
                    url: "<?php echo site_url('telco/get_reseller_route_tables'); ?>",
                    type : 'GET',
